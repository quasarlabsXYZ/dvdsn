...
func __setup__{
    ...
    %{
        ...
        context.SIMPLE_FLASH_LENDER = deploy_contract("utils/SimpleFlashLender.cairo", [ids.admin]).contract_address
        context.SIMPLE_FLASH_BORROWER = deploy_contract("utils/SimpleFlashBorrower.cairo", [context.SIMPLE_FLASH_LENDER]).contract_address
    %}
...
func check_addSupportedToken{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(){
    ...
    let (is_supported_before:felt) = ISimpleFlashLender.is_supported_token(SIMPLE_FLASH_LENDER,DVT);
...
func check_flashLoan{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(){
    ...
    let (local fees : Uint256) = ISimpleFlashLender.getFlashLoanFees(SIMPLE_FLASH_LENDER,DVT,max_amount);
