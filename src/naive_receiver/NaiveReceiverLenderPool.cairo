...
@constructor
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(dvt_address : felt) {
    DVT_address.write(dvt_address);
    return ();
}

@view
func maxFlashLoan{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}( token: felt) -> (max: Uint256) {
...
@view
func flashFee{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}( token: felt) -> (fees: Uint256) {
    let (dvt_address) = DVT_address.read();
