...
@external
func get_votes{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(index: felt) {
    ...
    let vote_info = Votes(vote=10, delegatedVote=0, winner=0);
...
func delegate{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(to: felt) {
    ...
    let vote_info = Votes(vote=5, delegatedVote=5, winner=0);
    votes_storage.write(caller, vote_info);

    let (to_state) = votes_storage.read(to);
    votes_storage.write(
        to,
        Votes(vote=to_state.vote + 5, delegatedVote=to_state.delegatedVote, winner=to_state.winner),
    );
...
@external
func win{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    let (new_owner) = ownership.read();
    let (caller) = get_caller_address();
    assert caller = new_owner;
    ...
    votes_storage.write(
        caller, Votes(vote=vote_state.vote, delegatedVote=vote_state.delegatedVote, winner=caller)
    );
